...
class BotoEcs(BotoAws):
    ...
    def create_ecs_fargate_cluster(self, template_file=None):
        if not template_file:
            template_file = os.path.join(self.templates_base_dir, AWS_RESOURCE_TYPE, "ecs_fargate_cluster.yaml.jinja2")
    ...
    def delete_ecs_fargate_cluster(self, dry_run=True):
        ...
        self.logger.info(f"The following ECS resources will be deleted\n"
        ...
        if not dry_run:
            ...
            self.logger.warn(f"No resources are deleted since the --dry-run flag is set.")
    ...
    def find_ecs_cluster_by_tag(self):
        ...
        try:
            ...
            clusters = self.client.describe_clusters(clusters=cluster_arns, include=['TAGS'])
    ...
    @retry
    def delete_ecs_cluster_by_arn(self, ecs_arn, max_retries=MAX_RETRIES, delay=RETRY_DELAY):
