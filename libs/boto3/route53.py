...
class BotoRoute53(BotoAws):
    ...
    def change_record_set_elbv2(self, action, elb_dns=None, template_file=None, dry_run=False):
        record_set_domain = self.input_values_dict.get('fargate_route_53').get('change_batch') \
            .get('changes')[0].get('resource_record_set').get('name')
        if dry_run:
            self.logger.info(f"The Route53 record set: {record_set_domain} will not be deleted "
                             "as the --dry-run flag is set")
        if action not in {"CREATE", "DELETE"}:
            raise ValueError(f"The action should either be CREATE or DELETE")
        template_file = self.get_template(template_file, CHANGE_RECORD_SET_TEMPLATE_FILE)
        if not elb_dns:
        ...
        try:
            ...
            self.logger.info(f"Performed action: {action} for record-set: {record_set_domain}")
