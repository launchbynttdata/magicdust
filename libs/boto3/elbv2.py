...
class BotoElbv2(BotoAws):
    ...
    def create_elbv2(self, subnet_ids=None, sg_id=None, template_file=None):
        if not template_file:
            template_file = os.path.join(self.templates_base_dir, AWS_RESOURCE_TYPE, "elbv2.yaml.jinja2")
        if not (subnet_ids or sg_id):
            boto_ec2 = BotoEc2(self.jinja_template, self.templates_base_dir)
            vpc_ids = boto_ec2.find_vpcs_by_tag()
            if not vpc_ids:
                raise Exception(f"No VPC Id found for the tag: {self.input_values_dict.get('tags').get('name')}")
    ...
    def create_elbv2_listeners(self, elbv2_arn=None, tg_arn=None, template_file=None):
        listener_arns = []
        if not template_file:
            template_file = os.path.join(self.templates_base_dir, AWS_RESOURCE_TYPE, "elbv2_listeners.yaml.jinja2")
        if not (elbv2_arn or tg_arn):
            elbv2_arns = self.find_elbv2_by_tag()
            if not elbv2_arns:
                raise Exception(f"No ELBv2 found for the tag: {self.input_values_dict.get('tags').get('name')}")
            elbv2_arn = elbv2_arns[0]
            tg_arns = self.find_elbv2_target_group_by_tag()
            if not tg_arns:
                raise Exception(f"No ELBv2 target group found for the tag: "
    ...
    def create_elbv2_target_group(self, vpc_id=None, template_file=None):
        if not template_file:
            template_file = os.path.join(self.templates_base_dir, AWS_RESOURCE_TYPE, "elbv2_target_group.yaml.jinja2")
        if not vpc_id:
            boto_ec2 = BotoEc2(self.jinja_template, self.templates_base_dir)
            vpc_ids = boto_ec2.find_vpcs_by_tag()
            if not vpc_ids:
    ...
    def delete_elbv2_resources(self, dry_run=True):
        ...
        elbv2_arns = self.find_elbv2_by_tag()
        tg_arns = self.find_elbv2_target_group_by_tag()
        self.logger.info(f"The following ELBv2 resources will be deleted\n"
    ...
    def find_elbv2_target_group_by_tag(self):
        ...
        try:
            ...
            all_tgs_arns = [tg.get('TargetGroupArn') for tg in all_tgs.get('TargetGroups')]
            ...
            self.logger.error(f"Exception occurred while finding Target Group by tag: {e}")
        return filtered_arns

    # Private functions

    @retry
    def delete_elbv2_by_arn(self, elbv2_arn, max_retries=MAX_RETRIES, delay=RETRY_DELAY):
