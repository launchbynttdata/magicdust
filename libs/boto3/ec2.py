...
class BotoEc2(BotoAws):
    ...
    def __init__(self, jinja_template, templates_base_dir, resource_type=AWS_RESOURCE_TYPE):
    ...
    def delete_vpc(self, dry_run=True):
        ...
        try:
            ...
            self.logger.info(f"Following resources are going to be deleted\n"
            ...
            if not dry_run:
                ...
                for igt_id in igt_ids:
                    self.client.detach_internet_gateway(InternetGatewayId=igt_id, VpcId=vpc_id)
                ...
                self.logger.info(f"No resources are deleted since the --dry-run flag is set.")
    ...
    def get_internet_gateways_by_vpc_id(self, vpc_id):
        ...
        try:
            ...
            igt_ids = [vpc.get('InternetGatewayId') for vpc in igts.get('InternetGateways')]
    ...
    def get_security_groups_by_vpc_id(self, vpc_id):
        ...
        try:
            ...
            sg_ids = [vpc.get('GroupId') for vpc in security_groups.get('SecurityGroups') if
                      vpc.get('GroupName') != "default"]
        except (KeyError, ClientError) as e:
    ...
    def get_subnets_by_vpc_id(self, vpc_id):
        ...
        try:
            ...
            subnet_ids = [subnet_id.get('SubnetId') for subnet_id in subnets.get('Subnets')]
        except (KeyError, ClientError) as e:
    ...
    def create_vpc(self, template_file=None):
        ...
        if len(existing_vpcs) > 0:
            raise Exception(f"VPCs: {existing_vpcs} already exists with the tag: "
                            f"{self.input_values_dict.get('tags').get('name')}. Please delete "
                            f"them or select different tag")
        if not template_file:
            template_file = os.path.join(self.templates_base_dir, AWS_RESOURCE_TYPE, "vpc.yaml.jinja2")
        request_string = self.jinja_template.generate_from_template(
            template_file, output_format="json", print_output=False)
        request_dict = json.loads(request_string)
        try:
    ...
    def create_security_group(self, vpc_id, template_file=None):
        if not template_file:
            template_file = os.path.join(self.templates_base_dir, AWS_RESOURCE_TYPE, "security_group.yaml.jinja2")
    ...
    def create_security_group_ingress(self, sg_id, template_file=None):
        if not template_file:
            template_file = os.path.join(self.templates_base_dir, AWS_RESOURCE_TYPE,
    ...
    @retry
    def delete_internet_gateway_by_id(self, igt_id, vpc_id, max_retries=MAX_RETRIES, delay=RETRY_DELAY):
        response = self.client.delete_internet_gateway(InternetGatewayId=igt_id)
        self.logger.debug(f"Response from API: {response.get('ResponseMetadata').get('HTTPStatusCode')}")

    @retry
    def delete_security_group_by_id(self, sg_id, max_retries=MAX_RETRIES, delay=RETRY_DELAY):
        response = self.client.delete_security_group(GroupId=sg_id)
        self.logger.debug(f"Response from API: {response.get('ResponseMetadata').get('HTTPStatusCode')}")

    @retry
    def delete_subnet_by_id(self, subnet_id, max_retries=MAX_RETRIES, delay=RETRY_DELAY):
